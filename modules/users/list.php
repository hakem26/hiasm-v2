...
const table = new Tabulator('#users-table', {
  ...
  columns: [
    ...
    { title: 'موبایل',     field: 'phone',        widthGrow: 1,
      formatter: v => v.getValue() || '—' },
    { title: 'نقش', field: 'role_label', widthGrow: 1,
      formatter: cell => {
        const map = { admin:'danger', leader:'warning', seller:'info' };
...
function toggleUser(id) {
  hiasm.post('<?= BASE_URL ?>/api/users.php', { action: 'toggle', id })
    .then(res => {
...
function deleteUser(id) {
  ...
  hiasm.post('<?= BASE_URL ?>/api/users.php', { action: 'delete', id })
    .then(res => {
      hiasm.toast(res.message, res.success ? 'success' : 'error');
      if (res.success) table.replaceData();
    });
