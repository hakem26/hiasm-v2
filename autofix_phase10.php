<?php
/**
 * این فایل api/customers.php را کامل بازنویسی می‌کند
 * با اضافه کردن action=search
 */

$target = __DIR__ . '/api/customers.php';
...
if (file_put_contents($target, $newContent)) {
    echo "<p style='color:green;font-family:Tahoma'>✓ api/customers.php بازنویسی شد</p>";
    echo "<p>تست کن: <a href='/hiasm-v2/api/customers.php?action=search&q=سم'>api/customers.php?action=search&q=سم</a></p>";
    echo "<p style='color:red'>این فایل را حذف کن!</p>";
} else {
    echo "<p style='color:red'>✗ نمی‌توانم فایل را بنویسم</p>";
}
